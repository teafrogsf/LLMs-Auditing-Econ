...
def main():
    ...
    parser.add_argument('--config', '-c', type=str, default='config/nl_graph/default.yaml')
    parser.add_argument('--output-dir', type=str, default='./outputs/nl_graph/3x0')
    ...
    # 策略组合：3-x-0，其中 provider 1 使用策略 3（我们的策略），provider 2 使用不同策略 x（0-9），provider 0 固定使用策略 0
    scenarios = [(2, x, 0) for x in range(10)]
