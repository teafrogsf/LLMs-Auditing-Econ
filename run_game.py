...
def init_config(args):
    ...
    config['reward_param'] = float(config['reward_param'])
...
def main():
    ...
    # parser.add_argument('--reward_param', '-r', type=float, default=1)
    # parser.add_argument('--gamma', '-g', type=float, default=5)
    parser.add_argument('--config', '-c', type=str, default='./config/nl_graph/yuhan1.yaml')
    parser.add_argument('--output-dir', type=str, default=None)
    parser.add_argument('--debug', action='store_true', default=False)
    args = parser.parse_args()
    
    if args.output_dir is None:
        args.output_dir = os.path.join('outputs/finnal', f'T{args.T}_eps{args.eps}') 
    ...
    os.makedirs(args.output_dir, exist_ok=True)
    logger = Logger(name=f'llm-game-{args.T}-{args.eps}-{args.output_dir}', path=os.path.join(args.output_dir, 'log.txt'))
    ...
    for sc in scenarios:
        ...
        if os.path.exists(os.path.join(config['output_dir'], 'result.json')):
            continue
