...
def run_single_task_eps(eps: int) -> None:
    output_dir = f"./outputs/toy_game/aba_eps3/eps{eps}"
    ...
    cmd = [
        ...
        output_dir,
        "--config",
        "config/toy_game/default.yaml"
    ]
    subprocess.run(cmd, check=True)

def run_single_task_T(T: int) -> None:
    output_dir = f"./outputs/toy_game/aba_T2/{T}"
    os.makedirs(output_dir, exist_ok=True)
    cmd = [
        "python",
        "run_toy_game.py",
...
def main() -> None:
    ...
    t_values = range(10, 20, 1)

    print(f"Launching {len(t_values)} tasks with max_workers={args.max_workers}...")
    failures = []
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        future_to_t = {executor.submit(run_single_task_eps, t): t for t in t_values}
