...
def create_three_main_visualizations(df):
    """创建三个主要可视化图表"""
    
    # 第一张图：三个provider的平均效用
    ...
    # 计算每个策略的平均效用
    strategy_means = utility_df.groupby('strategy')['utility'].mean().sort_values(ascending=False)
    
    bars = plt.bar(strategy_means.index, strategy_means.values, 
                   color=['gold', 'lightblue', 'lightcoral', 'lightgreen'])
    plt.title('各策略的服务商平均效用', fontsize=14, fontweight='bold')
    plt.ylabel('服务商效用', fontsize=12)
    plt.xlabel('策略类型', fontsize=12)
    ...
    # 第二张图：用户效用和回报的热力图（三维展示）
    plt.subplot(1, 3, 2)
    
    # 创建策略组合的矩阵
    strategies = ['honest', 'ours', 'worst', 'random']
    
    # 计算每个策略组合的用户效用和总回报
    user_utility_matrix = np.zeros((len(strategies), len(strategies)))
    total_reward_matrix = np.zeros((len(strategies), len(strategies)))
    
    for i, strategy1 in enumerate(strategies):
        for j, strategy2 in enumerate(strategies):
            # 找到所有包含这两个策略的组合
            subset = df[(df['provider1_strategy'] == strategy1) & (df['provider2_strategy'] == strategy2)]
            if not subset.empty:
                user_utility_matrix[i, j] = subset['total_user_utility'].mean()
                total_reward_matrix[i, j] = subset['total_reward'].mean()
            else:
                user_utility_matrix[i, j] = np.nan
                total_reward_matrix[i, j] = np.nan
    
    # 绘制用户效用热力图
    im = plt.imshow(user_utility_matrix, cmap='YlOrRd', aspect='auto')
    plt.colorbar(im, label='用户效用')
    plt.title('策略组合用户效用热力图\n(Provider1 vs Provider2)', fontsize=14, fontweight='bold')
    plt.xlabel('Provider2 策略', fontsize=12)
    plt.ylabel('Provider1 策略', fontsize=12)
    ...
    for i in range(len(strategies)):
        for j in range(len(strategies)):
            if not np.isnan(user_utility_matrix[i, j]):
                plt.text(j, i, f'{user_utility_matrix[i, j]:.0f}', 
    ...
    # 按策略分组计算平均综合评分
    comprehensive_scores = df.groupby('provider1_strategy')['comprehensive_score'].mean().sort_values(ascending=False)
    
    bars = plt.bar(comprehensive_scores.index, comprehensive_scores.values,
                   color=['gold', 'lightblue', 'lightcoral', 'lightgreen'])
    plt.title('各策略综合评分\n(用户效用+服务商效用+效率)', fontsize=14, fontweight='bold')
    plt.ylabel('综合评分', fontsize=12)
    plt.xlabel('策略类型', fontsize=12)
    plt.xticks(rotation=45)
    
    # 添加数值标签
    for bar, value in zip(bars, comprehensive_scores.values):
    ...
    return user_utility_matrix, total_reward_matrix, comprehensive_scores
...
def main():
    ...
    # 创建三个主要可视化
    print("创建主要可视化图表...")
    user_matrix, reward_matrix, comprehensive_scores = create_three_main_visualizations(df)
    ...
    print("主要图表已保存为 main_analysis.png")
    print("详细图表已保存为 detailed_simulation_analysis.png")
